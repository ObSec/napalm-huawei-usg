# -*- coding: utf-8 -*-
# Copyright 2023 Mario Oberli. All rights reserved.
#
...
"""
NAPALM Driver for Huawei USG Firewalls.
Author: Mario Oberli (mario.oberli@obsec.ch)
Maintainers: Mario Oberli (mario.oberli@obsec.ch)
...
class USGDriver(NetworkDriver):
    """Napalm driver for Huawei USG."""
    ...
    def get_facts(self):
        ...
        show_device_domain = self.device.send_command('display current-configuration | include device-domain')

        # os_version/uptime/model
        for line in show_ver.splitlines():
            if 'VRP (R) Software' in line:
                search_result = re.search(r"\(USG\S+\s+(?P<os_version>V\S+)\)", line)
                if search_result is not None:
                    os_version = search_result.group('os_version')

            if 'USG' in line and 'uptime is' in line:
                search_result = re.search(r"USG\S+", line)
                if search_result is not None:
                    model = search_result.group(0)
                uptime = self._parse_uptime(line)
                break

        # get serial_number, should only get one, still returing it as a list
        re_sn = r"ESN\s+of\s+master:(?P<serial_number>\S+)"
        ...
        if 'device-domain ' in show_device_domain:
            _, fqdn = show_device_domain.split("device-domain ")
            fqdn = fqdn.strip()
    ...
    def get_environment(self):
        ...
        -------------------------------------------------------------------------------
             FAN1     [1]     Present  Registered    60%(11315)   AUTO    Front-to-Back
        """
        power_cmd = 'display power'
        """
        --------------------------------------------------------------------------------------
         PowerID  Online  Mode  State      Rated Current(A)  Rated Voltage(V)  Rated Power(W)
        --------------------------------------------------------------------------------------
        ...
        CPU Usage            :   8.9%  Max:  54.6%
        Management-plane CPU Usage:  20.2%   Data-plane CPU Usage :   6.2%
        CPU utilization for ten seconds:   8.9% : one minute:   8.9% : five minutes:   9.0%
        """
        mem_cmd = 'display memory-usage'
        """
         Memory utilization statistics at 2023-07-31 20:56:02+02:00 DST
         System Total Memory Is: 419430400 bytes
         Total Memory Used Is: 299965880 bytes
        ...
        # device fan condition
        environment.setdefault('fans', {})
        for i in fan_output.split('\n'):
            match = re.match(r"\s+(FAN\d)\s+\S+\s+(Present|Abnormal).+", i)
            if match:
                slot = match.group(1)
                status = True if match.group(2) == "Present" else False
                environment['fans'][slot] = {'status': status}

        # device power status
        environment.setdefault('power', {})
        for i in power_cmd.split('\n'):
            match = re.match(r"\s(\d)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\d+)\s+(\d+)\s+(\d+)", i)
            if match:
                environment['power'][f"PWR{match.group(1)}"] = {
                    "current": (match.group(5))+"V",
                    "voltage": (match.group(6))+"W",
                    "power": (match.group(7))+"W",
                    "status": True if match.group(2) == 'Present' else False
        ...
        for i in temp_cmd.split('\n'):
            match1 = re.match('\s+\d\s+-\s+(\d)\s+(\S+)\s+(\d+)\s+\d\s+\d+', i)
            match = re.match('\s+-\s+(\d)\s+(\S+)\s+(\d+)\s+\d+\s+\d+', i)
            if match1:
                environment['temperature'][f"sensor{match1.group(1)}"] = {
                    "is_alert": False if match1.group(2) == "Normal" else True,
                    "is_critical": False if match1.group(2) == "Normal" else True,
                    "temperature": match1.group(3)
                }
            if match:
                environment['temperature'][f"sensor{match.group(1)}"] = {
                    "is_alert": False if match.group(2) == "Normal" else True,
                    "is_critical": False if match.group(2) == "Normal" else True,
    ...
    def get_mac_address_table(self):
        ...
        re_mac = r"(?P<mac>\S+)\s+(?P<vlan>\d+)\s+(?P<timer>\d+)\s+(?P<interface>\S+)\s+(?P<type>\w+)\s+"
        match = re.findall(re_mac, output, re.M)

        for mac_info in match:
            mac_dict = {
                'mac': napalm.base.helpers.mac(mac_info[0]),
                'interface': mac_info[3],
                'vlan': int(mac_info[1]),
                'static': True if mac_info[4] == "static" else False,
                'active': True if mac_info[4] == "dynamic" else False,
                'authen': True if mac_info[4] == "authen" else False,
                'moves': -1,
                'last_move': -1.0
            }
